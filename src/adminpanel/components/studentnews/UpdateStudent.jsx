...
const UpdateStudentNews = ({ setActiveComponent,studentData }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.put(`${url}/api/studentnews/${studentData.id}`, formData);
      if(res.status===200)
      {
        toast.success(res.data.message)
        setActiveComponent("Student Corner")
      }
    } catch (error) {
      console.log(error);
    }
...
export default UpdateStudentNews;
